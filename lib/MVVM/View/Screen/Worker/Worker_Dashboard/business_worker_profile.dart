...
class BusinessWorkerProfile extends StatelessWidget {
  const BusinessWorkerProfile({super.key});

  @override
  Widget build(BuildContext context) {
    // Initialize or find the controller
    final BusinessDashboardController controller =
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Obx(() {
          ...
          final userName =
              data['username'] ?? data['facility_name'] ?? 'Business';
          final phone = data['phone'] ?? data['contact_number'] ?? 'N/A';
          final email = data['email'] ??
              FirebaseAuth.instance.currentUser?.email ??
              'N/A';
          final category =
              data['profession'] ?? data['category'] ?? 'Business Operator';
          final availableTime = data['available_time'] ?? 'N/A';
                  ...
                  children: [
                    Container(
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color: const Color(0xFFF9A825), // Gold border
                            width: 3),
                      ),
                      child: CircleAvatar(
                        radius: 40,
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildInfoCard(
                            ...
                            children: [
                              _buildDetailRow('FACILITY NAME', userName),
                              const Divider(),
                          ...
                          _buildSettingsTile(
                            ...
                            // Get.to(() => const EditBusinessWorkerProfile());
                          }),
                          _buildSettingsTile(
