...
// ─────────────────────────────────────────────────
int smartRank(JcbsListing a, JcbsListing b, double userLat, double userLng,
    String userLocality) {
  final dA = a.distanceFrom(userLat, userLng);
  final dB = b.distanceFrom(userLat, userLng);

  // 1. Nearest JCB stands (comparing distance first, with a small 0.3 km tolerance to allow tie-breaking)
...
// Page
// ─────────────────────────────────────────────────
class JcbsPage extends StatefulWidget {
  const JcbsPage({Key? key}) : super(key: key);

  @override
  State<JcbsPage> createState() => _JcbsPageState();
}

class _JcbsPageState extends State<JcbsPage>
  ...
  Future<void> _fetchListingsFromFirestore() async {
    try {
      ...
      debugPrint("Error fetching JCB listings: $e");
      if (mounted) {
        setState(() {
          _isLoadingListings = false;
        });
  ...
  List<JcbsListing> get _rankedListings {
    List<JcbsListing> result = _allListings.where((item) {
  ...
  Widget _buildListingCard(JcbsListing item) {
    final dist = item.distanceFrom(_userLat, _userLng);
  ...
  Widget _callIconButton(JcbsListing item) {
    return Container(
      height: 40,
      width: 40,
      decoration: BoxDecoration(
  ...
  void _openDetails(JcbsListing item) {
    Navigator.push(
        context,
        MaterialPageRoute(
            builder: (_) =>
  ...
  void _showInfoDialog() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: const Text("JCB & JCB Guide",
            style: TextStyle(
