...
// ─────────────────────────────────────────────────
int smartRank(AutoTaxiListing a, AutoTaxiListing b, double userLat,
    double userLng, String userLocality) {
  final dA = a.distanceFrom(userLat, userLng);
  final dB = b.distanceFrom(userLat, userLng);

  // 1. Nearest taxi stands (comparing distance first, with a small 0.3 km tolerance to allow tie-breaking)
...
class AutoTaxiPage extends StatefulWidget {
  const AutoTaxiPage({Key? key}) : super(key: key);
...
class _AutoTaxiPageState extends State<AutoTaxiPage>
  ...
  Future<void> _loadMoreListings() async {
    ...
    try {
      ...
      debugPrint("Error loading more taxi listings: $e");
      if (mounted) {
        setState(() {
          _isFetchingMore = false;
        });
  ...
  Future<List<AutoTaxiListing>> _processSnapshot(QuerySnapshot snapshot) async {
  ...
  Widget _buildTypeFilterChips() {
    const types = ['All', 'Auto', '4-Wheel Taxi', 'Van'];
    return SizedBox(
      height: 38,
      child: ListView(
        scrollDirection: Axis.horizontal,
  ...
  Widget _buildListingCard(AutoTaxiListing item) {
    final dist = item.distanceFrom(_userLat, _userLng);
  ...
  Widget _callIconButton(AutoTaxiListing item) {
    return Container(
      height: 40,
      width: 40,
      decoration: BoxDecoration(
  ...
  void _openDetails(AutoTaxiListing item) {
    Navigator.push(context,
        MaterialPageRoute(builder: (_) => VehicleDetailsPage(listing: item)));
  }
