...
class PickupMapView extends StatefulWidget {
  final List<PickupListing> listings;
  final double userLat;
  final double userLng;
  final void Function(String phone) onCallTap;
  final void Function(PickupListing item) onDetailsTap;
...
class _PickupMapViewState extends State<PickupMapView> with TickerProviderStateMixin {
  PickupListing? _selected;
  ...
  void initState() {
    ...
    _sweepCtrl = AnimationController(vsync: this, duration: const Duration(seconds: 3))
      ..repeat();
  }
      ...
      child: Container(
        ...
        child: LayoutBuilder(
          builder: (ctx, constraints) {
            final center = Offset(constraints.maxWidth / 2, constraints.maxHeight / 2) + _panOffset;
                  ...
                  child: Stack(
                    children: [
                      ...
                      ...widget.listings.map((v) {
                        final pos = _latlngToOffset(v.latitude, v.longitude, center, _scale);
                        return Positioned(
                          left: pos.dx - 20,
                          top: pos.dy - 20,
                          child: GestureDetector(
                            onTap: () => setState(() => _selected = _selected == v ? null : v),
                            child: SizedBox(
                              width: 40,
                              height: 40,
                              child: _PickupMapMarker(
                                item: v,
  ...
  Widget _buildSelectedCard(PickupListing item) {
    final dist = item.distanceFrom(widget.userLat, widget.userLng);
    final eta = item.etaMinutes(widget.userLat, widget.userLng);
    final distStr = dist < 1 ? '${(dist * 1000).round()} m away' : '${dist.toStringAsFixed(1)} km away';
              ...
              Container(
                ...
                child: Icon(
                  item.isAgency
                      ...
                      : (item.type.toLowerCase().contains('jcb')
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        if (item.isVerified)
                          const Icon(Icons.verified_user_rounded, color: Color(0xFF4F46E5), size: 14),
                      ],
                    ),
              ...
              Column(
                ...
                children: [
                  Text(
                    distStr,
                    style: const TextStyle(
                        color: Color(0xFF0F2E5A), fontWeight: FontWeight.bold, fontSize: 13),
                  ),
                  Text(
                    '~$eta min away',
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: const Icon(Icons.arrow_forward_ios_rounded, color: Color(0xFF0F2E5A), size: 16),
                ),
              ),
            ],
          ),
...
// ─────────────────────────────────────────────────
class _PickupMapMarker extends StatelessWidget {
  final PickupListing item;
  final bool isSelected;

  const _PickupMapMarker({required this.item, required this.isSelected});
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: (isSelected ? const Color(0xFFFFB800) : const Color(0xFF0F2E5A)).withOpacity(0.5),
      ...
      child: Icon(
        item.type.toLowerCase().contains('jcb')
            ...
            : item.isAgency
                ? Icons.domain
                : Icons.agriculture,
        color: isSelected ? const Color(0xFF0F2E5A) : const Color(0xFFFFB800),
        size: 18,
      ),
    );
  }
